...
@section('content')
    <div class="container py-4">
        ...
        <div class="row">
            <div class="col">
                <div class="btn-group" role="group" aria-label="Basic example">
                    <a href="{{route('admin.farms.index',['select' => 'withTrashed'])}}" type="button"
                       class="btn btn-outline-secondary {{request()->get('select') == 'withTrashed'  ? 'active' : ''}}">Все</a>
                    <a href="{{route('admin.farms.index',['select' =>'withoutTrashed'])}}" type="button"
                       class="btn btn-outline-secondary {{(request()->get('select') == 'withoutTrashed' || request()->get('select') == null)? 'active' : ''}}">Активные</a>
                    <a href="{{route('admin.farms.index',['select' => 'trashed']) }}" type="button"
        ...
        <div class="row">
            <div class="col">
                @include('admin.farms.partials.farms-table')
            </div>
        </div>
        <div class="row mt-4">
            <div class="col">
